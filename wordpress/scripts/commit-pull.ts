...
for (const location of ['content', 'src/data/site-settings.json', 'archive/wordpress-deleted']) {
  // A checkout may have no content directory after its last article was archived.
  if (run(['status', '--porcelain', '--untracked-files=all', '--', location]).trim()) {
    run(['add', '-A', '--', location]);
  }
}
